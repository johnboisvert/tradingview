...
function publishArticle({ title, excerpt, content, tags }, { getNewsletterNotifier } = {}) {
...
async function runPublishCycle({ getNewsletterNotifier, dryRun = false, forcedTopic = null } = {}) {
  ...
  try {
    ...
    const article = publishArticle(gen, { getNewsletterNotifier });
    const entry = { date: todayKey(), ts: startTs, ok: true, topic, slug: article.slug, title: article.title, tokens: gen.usage?.total_tokens || null };
    recordRun(entry);
    console.log(`[BlogCron] ✅ Published "${article.title}" (${article.slug})`);
...
export { runPublishCycle, generateArticle, getNextTopic };
