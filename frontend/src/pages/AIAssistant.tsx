...
- Si on te pose une question hors sujet crypto/trading, ramène poliment la conversation vers ton domaine d'expertise`;
      ...
      await client.ai.gentxt({
        ...
        onError: (error: { message?: string }) => {
          ...
          setMessages((prev) => [
            ...
            {
              role: "assistant",
              content: `⚠️ Erreur de connexion IA: ${error?.message || "Erreur inconnue"}. Veuillez réessayer.`,
            },
          ]);
          setStreamingContent("");
          setIsTyping(false);
  ...
  const renderContent = (content: string) => {
    ...
    return content.split("\n").map((line, i) => {
      ...
      if (line.startsWith("• ") || line.startsWith("- ")) {
        return (
          <p key={i} className="ml-2 my-0.5" dangerouslySetInnerHTML={{ __html: boldLine }} />
        );
      }
          ...
          <div className="relative z-10 h-full flex items-center justify-between px-8">
            <div>
              ...
              <p className="text-sm text-gray-400">
                Propulsé par DeepSeek V3 • Réponses intelligentes en temps réel • Streaming
              </p>
            </div>
            <button
              onClick={clearChat}
        ...
        <div className="flex-1 bg-[#111827] border border-white/[0.06] rounded-2xl flex flex-col overflow-hidden">
          ...
          <div className="p-4 border-t border-white/[0.06]">
            ...
            <p className="text-[10px] text-gray-600 mt-2 text-center">
              Propulsé par DeepSeek V3 • Les réponses ne constituent pas des conseils financiers
            </p>
          </div>
        </div>
      </main>
