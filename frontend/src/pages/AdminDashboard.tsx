...
export default function AdminDashboard() {
  ...
  const adminLinks = [
    { path: "/admin/health", label: "Health Monitor", icon: Activity, desc: "Services & uptime", color: "from-emerald-500 to-teal-600" },
  ...
  return (
    <AdminLayout>
      {/* HERO Header — premium glass + animated gradient */}
      <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-br from-slate-900/90 via-indigo-950/60 to-slate-900/90 p-6 sm:p-8 mb-6">
        ...
        <div className="relative flex items-start justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-400 via-orange-500 to-red-500 flex items-center justify-center shadow-2xl shadow-amber-500/30">
              <Shield className="w-7 h-7 text-white" />
              <div className="absolute -inset-1 rounded-2xl border-2 border-amber-400/40 animate-pulse pointer-events-none" />
            </div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-amber-300/80 mb-1">Control Center</p>
              <h1 className="text-2xl sm:text-3xl font-black tracking-tight bg-gradient-to-r from-white via-white to-white/70 bg-clip-text text-transparent">
                Admin Dashboard
              </h1>
              <p className="text-xs text-slate-400 mt-1">Vue d'ensemble de la plateforme CryptoIA</p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <Link
              to="/admin/health"
              data-testid="admin-quicklink-health"
              className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs font-black text-emerald-300 hover:bg-emerald-500/20 transition-all"
            >
              <Activity className="w-3.5 h-3.5" /> Health Monitor
            </Link>
            <span className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[11px] font-bold text-emerald-300">
