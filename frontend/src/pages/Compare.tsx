...
function CompareList() {
  ...
  useEffect(() => {
    ...
    document.title = "Comparatifs Crypto — CryptoIA";
  }, []);
  return (
      ...
      <main className="flex-1 px-6 py-10 max-w-5xl mx-auto" data-testid="compare-list">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/30 mb-4">
            ...
            <span className="text-[11px] uppercase tracking-widest font-black text-purple-300">Comparatifs</span>
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black mb-3 bg-gradient-to-r from-purple-300 via-pink-300 to-rose-300 bg-clip-text text-transparent">Comparatifs Crypto</h1>
          <p className="text-base text-gray-400 max-w-2xl mx-auto">{items.length} comparatifs objectifs pour t&apos;aider à choisir selon ton profil.</p>
        </div>
        ...
        {!loading && items.length === 0 && (
          <p className="text-center text-gray-500 py-12" data-testid="compare-empty">
            Les comparatifs seront bientôt disponibles. <Link to="/admin/seo-content" className="text-purple-400 hover:underline">Admin : générer →</Link>
          </p>
        )}
          ...
          <article>
                ...
                <table className="w-full text-sm border-collapse" data-testid="compare-criteria-table">
                  <thead>
                    <tr className="border-b border-white/10">
                      <th className="text-left py-3 px-3 text-[11px] uppercase tracking-wider font-black text-gray-400">Critère</th>
                      ...
                      <th className="text-center py-3 px-3 text-[11px] uppercase tracking-wider font-black text-gray-400">Gagnant</th>
                    </tr>
                  </thead>
                  <tbody>
                    {item.criteria.map((c, i) => (
                      <tr key={i} className="border-b border-white/[0.04] align-top">
                        ...
                        <td className="py-3 px-3 text-center">
                          ...
                          {(c.winner === 'égalité' || c.winner === 'egalité' || !c.winner) && <span className="text-gray-500 text-xs">Égalité</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {item.verdict && (
              <div className="rounded-2xl border border-purple-500/30 bg-purple-500/[0.07] p-6 mb-8">
                <h2 className="text-lg font-black text-purple-200 mb-3 flex items-center gap-2"><Trophy className="w-4 h-4" /> Verdict</h2>
                <div className="text-gray-200 leading-relaxed" dangerouslySetInnerHTML={{ __html: markdownToHtml(item.verdict) }} />
              </div>
            )}
            {item.faq && item.faq.length > 0 && (
              <div className="mt-8">
                <h3 className="text-sm uppercase tracking-wider text-gray-400 font-black mb-3 flex items-center gap-2"><HelpCircle className="w-4 h-4" /> FAQ</h3>
