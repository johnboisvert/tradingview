//@version=6
indicator("Magic Mike Pro V3.1 - AVEC CONFIANCE (0-100%)", overlay=true, max_labels_count=500, max_lines_count=200, max_boxes_count=80)
...
if tg_enable and earlyBuyAlert
    ...
    payload = "{\"type\":\"EARLY_ALERT\",\"symbol\":\"" + cleanSymbol + "\",\"side\":\"LONG\",\"current_price\":" + str.tostring(close, format.mintick) + ",\"tf\":\"" + timeframe.period + "\",\"confidence\":" + str.tostring(confidencePct) + ",\"message\":\"Setup en formation - Préparez-vous\"}"
    alert(payload, alert.freq_once_per_bar_close)

if tg_enable and earlySellAlert
    cleanSymbol = str.replace(syminfo.ticker, ".P", "")
    payload = "{\"type\":\"EARLY_ALERT\",\"symbol\":\"" + cleanSymbol + "\",\"side\":\"SHORT\",\"current_price\":" + str.tostring(close, format.mintick) + ",\"tf\":\"" + timeframe.period + "\",\"confidence\":" + str.tostring(confidencePct) + ",\"message\":\"Setup en formation - Préparez-vous\"}"
    alert(payload, alert.freq_once_per_bar_close)

if tg_enable and buyTrig
    cleanSymbol = str.replace(syminfo.ticker, ".P", "")
    payload = "{\"type\":\"ENTRY\",\"symbol\":\"" + cleanSymbol + "\",\"side\":\"LONG\",\"entry\":" + str.tostring(lastEntry, format.mintick) + ",\"current_price\":" + str.tostring(close, format.mintick) + ",\"sl\":" + str.tostring(lastSL, format.mintick) + ",\"tp1\":" + str.tostring(lastTP1, format.mintick) + ",\"tp2\":" + str.tostring(lastTP2, format.mintick) + ",\"tp3\":" + str.tostring(lastTP3, format.mintick) + ",\"tf\":\"" + timeframe.period + "\",\"confidence\":" + str.tostring(confidencePct) + "}"
    alert(payload, alert.freq_once_per_bar_close)

if tg_enable and sellTrig
    cleanSymbol = str.replace(syminfo.ticker, ".P", "")
    payload = "{\"type\":\"ENTRY\",\"symbol\":\"" + cleanSymbol + "\",\"side\":\"SHORT\",\"entry\":" + str.tostring(lastEntry, format.mintick) + ",\"current_price\":" + str.tostring(close, format.mintick) + ",\"sl\":" + str.tostring(lastSL, format.mintick) + ",\"tp1\":" + str.tostring(lastTP1, format.mintick) + ",\"tp2\":" + str.tostring(lastTP2, format.mintick) + ",\"tp3\":" + str.tostring(lastTP3, format.mintick) + ",\"tf\":\"" + timeframe.period + "\",\"confidence\":" + str.tostring(confidencePct) + "}"
    alert(payload, alert.freq_once_per_bar_close)
